...
export function useTags(params?: PaginationParams & { search?: string; category?: string }) {
  return useQuery({
    ...
    queryFn: async (): Promise<PaginatedResponse<ApiTag>> => {
      const response = await apiClient.get<PaginatedResponse<ApiTag> | ApiTag[]>(
        '/tags',
      ...
      if (data && typeof data === 'object' && 'data' in data) {
        return data as PaginatedResponse<ApiTag>;
      }
...
export function useAllTags() {
  return useQuery({
    ...
    queryFn: async (): Promise<ApiTag[]> => {
      const response = await apiClient.get<ApiTag[] | { data: ApiTag[] }>(
        '/tags',
        { params: { limit: 1000 } }
      );
      const data = response.data;
...
export function useTag(id: string) {
  return useQuery({
    ...
    queryFn: async (): Promise<ApiTag> => {
      const response = await apiClient.get<ApiTag>(`/tags/${id}`);
...
export function usePopularTags(limit = 10) {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await apiClient.get<ApiTag[]>('/tags/popular', { params: { limit } });
      return response.data || [];
    },
    staleTime: 15 * 60 * 1000,
    gcTime:30 * 60 * 1000,
...
export function useTagsByCategory() {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await apiClient.get<Record<string, ApiTag[]>>('/tags/by-category');
      return response.data || {};
    },
    staleTime: 15 * 60 * 1000,
    gcTime:30 * 60 * 1000,
