...
export const partialLeadService = {
  ...
  async convertPartialLead(partialLeadId: string): Promise<boolean> {
    try {
      const response = await apiClient.post<unknown>(`/partial-leads/${partialLeadId}/convert`);
      return response.success;
    } catch (error) {
  ...
  async markAsAbandoned(partialLeadId: string): Promise<boolean> {
    try {
      const response = await apiClient.post<unknown>(`/partial-leads/${partialLeadId}/abandon`);
      return response.success;
    } catch (error) {
